...
var doc=document,flower=doc.querySelector('.flower'),petalPartMarkup='<div class="box"><div class="shape"></div></div>',maxParts=20,maxPetals=6,partsFontStep=25/maxParts;createFlower();function createFlower(){var angle=360/maxPetals;for(var i=0;i<maxPetals;i++){var petal=createPetal(),currAngle=angle*i+'deg',transform='transform: rotateY('+currAngle+') rotateX(-30deg) translateZ(9vmin)';petal.setAttribute('style',transform);flower.appendChild(petal);}}function createPetal(){var box=createBox(null,0),petal=doc.createElement('div');petal.classList.add('petal');for(var i=1;i<=maxParts;i++){box=createBox(box,i);}petal.appendChild(box);return petal;}function createBox(box,pos){var fontSize=partsFontStep*(maxParts-pos)+'vmin',half=maxParts/2,bright='50';if(pos<half+1){fontSize=partsFontStep*pos+'vmin';}else{bright=10+40/half*(maxParts-pos);}var baseHue=280,hueVariation=30,saturation=75+(20*pos/maxParts),color='hsl('+(baseHue+(hueVariation*pos/maxParts))+', '+saturation+'%, '+bright+'%)',newShape=doc.createElement('div');newShape.classList.add('shape');var newBox=doc.createElement('div');newBox.classList.add('box');newBox.setAttribute('style','color: '+color+';font-size: '+fontSize);if(box)newBox.appendChild(box);newBox.appendChild(newShape);return newBox;}function drawGalaxy(){var canvas=document.getElementById('galaxy-canvas');if(!canvas)return;var ctx=canvas.getContext('2d');function resize(){canvas.width=window.innerWidth;canvas.height=window.innerHeight;}resize();window.addEventListener('resize',resize);var stars=[],numStars=120;for(var i=0;i<numStars;i++){stars.push({x:Math.random()*canvas.width,y:Math.random()*canvas.height,r:Math.random()*1.5+0.5,dx:(Math.random()-0.5)*0.7,dy:(Math.random()-0.5)*0.7,alpha:Math.random()*0.5+0.5});}function animate(){ctx.clearRect(0,0,canvas.width,canvas.height);for(var i=0;i<stars.length;i++){var s=stars[i];ctx.save();ctx.globalAlpha=s.alpha;ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fillStyle='rgba(255,182,193,0.9)';ctx.shadowColor='#ffb6d5';ctx.shadowBlur=2;ctx.fill();ctx.restore();s.x+=s.dx;s.y+=s.dy;if(s.x<0||s.x>canvas.width)s.dx*=-1;if(s.y<0||s.y>canvas.height)s.dy*=-1;}requestAnimationFrame(animate);}animate();}document.addEventListener('DOMContentLoaded',drawGalaxy);
  ...
  const messages = [
    ...
    'Hemos abierto nuestro pasado',
    'para encontrar un poco de paz.',
    'Déjame escuchar tu corazón,',
    'se que has llorado.',
    'Quisiera estar a tu lado,',
    'y confío en que me ves tal como soy.',
    'Tengo mil y un formas',
    'de criticar mis pecados,',
    'pero a tu lado',
    'siempre pesaron menos.',
    'Pequeña estrella,',
    'ya has crecido,',
    'o mejor dicho,',
    'ambos hemos cambiado.',
    'Sabes muy bien que ahora',
    'no puedo decirlo,',
    'porque ambos moriríamos.',
    'Pero espero que al verte,',
    'cumplamos un sueño',
    'sacado de cuentos.',
    'Gracias por brillar en mi cielo.',
  ...
  startBtn.addEventListener('click', function() {
      ...
      setTimeout(function() {
        ...
        function showNext() {
          if (current < messages.length) {
            ...
            // MOSTRAR CARTA COMPLETA AL FINAL
            showCompleteLetter();
    ...
    // FUNCIÓN PARA MOSTRAR CARTA COMPLETA
    function showCompleteLetter() {
      setTimeout(function() {
        // Crear contenedor para la carta completa
        var completeLetter = document.createElement('div');
        completeLetter.id = 'complete-letter';
        completeLetter.style.cssText = `
          position: fixed;
          top: 50%;
          left: 50%;
        ...
        // Crear el contenido de la carta completa
        ...
        letterContent.textContent = fullText;
        ...
        closeBtn.onclick = function() {
          document.body.removeChild(completeLetter);
        };

        // Agregar elementos al contenedor
        completeLetter.appendChild(letterContent);
        completeLetter.appendChild(closeBtn);
        
        // Agregar al documento
        document.body.appendChild(completeLetter);

        // Efecto de aparición
        completeLetter.style.opacity = '0';
        completeLetter.style.transition = 'opacity 0.5s ease-in-out';
        
        setTimeout(function() {
          completeLetter.style.opacity = '1';
        }, 100);
